...
/// error from the call to `/upload_session/finish_batch/check`. The third argument is the general network error from
/// `/upload_session/finish_batch/check`. This object will be nonnull if there is a network error from the call to
/// `/upload_session/finish_batch/check`. The fourth argument is a mapping of client-side NSURLs to general network
...
typedef void (^DBBatchUploadResponseBlock)(
    ...
    DBASYNCPollError * _Nullable finishBatchRouteError, DBRequestError * _Nullable finishBatchNetworkError,
    NSDictionary<NSURL *, DBRequestError *> * _Nonnull fileUrlsToNetworkErrors);
