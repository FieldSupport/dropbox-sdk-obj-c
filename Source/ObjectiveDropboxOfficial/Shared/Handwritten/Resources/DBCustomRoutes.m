  ...
  __block DBUploadTask *task = [[[self uploadSessionStartUrl:@(YES) inputUrl:fileUrl]
      setResponseBlock:^(DBFILESUploadSessionStartResult *result, DBNilObject *routeError, DBRequestError *error) {
        if (result && !routeError) {
          ...
          uploadData.fileUrlsToNetworkErrors[fileUrl] = error;
        }
        ...
        if (result) {
          if ([result isInProgress]) {
            ...
            if (retryCount <= timeoutInSec) {
              ...
              [uploadData.queue addOperationWithBlock:^{
                uploadData.responseBlock(nil, nil, [[DBRequestError alloc] initAsClientError:timeoutError],
                                         uploadData.fileUrlsToNetworkErrors);
              }];
            }
            ...
            [uploadData.queue addOperationWithBlock:^{
              ...
              uploadData.responseBlock(fileUrlsToBatchResultEntries, nil, nil, uploadData.fileUrlsToNetworkErrors);
            }];
          }
        } else if (!routeError) {
          if ([error isRateLimitError]) {
            ...
            [uploadData.queue addOperationWithBlock:^{
              uploadData.responseBlock(nil, nil, error, uploadData.fileUrlsToNetworkErrors);
            }];
          }
        } else {
          [uploadData.queue addOperationWithBlock:^{
            uploadData.responseBlock(nil, routeError, error, uploadData.fileUrlsToNetworkErrors);
          }];
        }
      }];
}
